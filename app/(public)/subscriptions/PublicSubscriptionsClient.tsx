...
export default function PublicSubscriptionsClient({ savedPaymentMethods, userName, isAuthed }: IPublicSubscriptionsClient) {
  const router = useRouter()
  const searchParams = useSearchParams()

  // The name and saved cards came from the server, so a sign-out while the page is open reloads it
  ...
  // Every view change goes through here, so the address always matches what's on screen after a refresh or back
  ...
  return (
    <main id="main-content" className="dark relative min-h-dvh text-text-dark bg-bg-dark">
      <StickyHeader billing={billing} selected={selected} onSubscribe={() => handleView('payment')} view={view} />
