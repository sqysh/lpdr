...
export function WinnerOrderSummary({ winningBidder, total, shipping, processingFee, finalAmount, coverFees }: Props) {
  return (
      ...
      <div className="border border-zinc-200 dark:border-border-dark">
        ...
        <div className="px-5 py-4 border-b border-zinc-200 dark:border-border-dark">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase tracking-[0.25em] text-zinc-500 dark:text-muted-dark">Order Summary</span>
          </div>
        </div>
          ...
          {winningBidder.auctionItems.map((item) => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-3 px-5 py-4">
                ...
                <span className="shrink-0 text-sm tabular-nums text-zinc-950 dark:text-text-dark">${item.soldPrice.toLocaleString()}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="px-5 py-4 border-t border-zinc-200 dark:border-border-dark space-y-2.5">
          ...
          {shipping > 0 && (
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs tabular-nums text-zinc-950 dark:text-text-dark">${shipping.toLocaleString()}</span>
            </div>
          )}
          {processingFee > 0 && coverFees && (
            <div className="flex justify-between items-center">
              <span className="font-lato text-xs text-zinc-500 dark:text-muted-dark">Processing fee</span>
              <span className="text-xs tabular-nums text-zinc-950 dark:text-text-dark">${processingFee.toLocaleString()}</span>
            </div>
          )}
          <div className="pt-3 border-t border-zinc-200 dark:border-border-dark flex justify-between items-center">
            ...
            <span className="text-xl tabular-nums text-cyan-600 dark:text-violet-400">${finalAmount.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </motion.div>
