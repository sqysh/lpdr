...
const panelHead = 'px-5 py-4 border-b border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark'

const panelLabel = 'text-[10px] font-mono tracking-[0.2em] uppercase text-muted-light dark:text-muted-dark'

const rowLabel = 'text-[10px] font-mono tracking-[0.15em] uppercase text-muted-light dark:text-muted-dark shrink-0'
...
export default function MyPackSubscriptionClient({ subscription }: { subscription: NonNullable<Subscription> }) {
  ...
  const overviewRows = [
    ...
    },
    {
      ...
      value: (
        <span
          className={`text-[10px] font-mono tracking-tag uppercase ${
      ...
      <main id="main-content" className="min-h-screen bg-bg-light dark:bg-bg-dark text-text-light dark:text-text-dark">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-12 sm:pt-16 pb-24 sm:pb-32">
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="show" custom={0} className="mb-10">
            <Link
              href="/my-pack?tab=giving"
              className="inline-flex items-center gap-2 text-[10px] font-mono tracking-eyebrow uppercase text-muted-light dark:text-muted-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors focus:outline-none focus-visible:underline mb-8"
            >
              <LinkBody icon={<ArrowLeft className="w-3.5 h-3.5" aria-hidden="true" />} label="My Pack" />
            </Link>

            <div className="flex items-center gap-3 mb-3">
              <span className="block w-6 h-px bg-primary-light dark:bg-primary-dark shrink-0" aria-hidden="true" />
              <p className="text-xs font-mono tracking-eyebrow uppercase text-primary-light dark:text-primary-dark">Subscription</p>
            </div>
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              {isCancelled && (
                <span className="shrink-0 mt-2 px-2.5 py-1 border border-red-500/30 text-[9px] font-mono tracking-tag uppercase text-red-500 dark:text-red-400">
                  Cancelled
                </span>
              )}
            </div>
          ...
          {/* ── Overview card ── */}
          <motion.div variants={fadeUp} initial="hidden" animate="show" custom={1} className={`${panel} mb-6`}>
          ...
          {/* ── Payment method ── */}
          <motion.div variants={fadeUp} initial="hidden" animate="show" custom={2} className={`${panel} mb-6`}>
            <div className={`${panelHead} flex items-center justify-between`}>
              ...
              {!isCancelled && !showUpdateCard && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-[10px] font-mono tracking-eyebrow uppercase text-muted-light dark:text-muted-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors focus:outline-none focus-visible:underline"
                >
          ...
          {/* ── Billing history ── */}
          <motion.div variants={fadeUp} initial="hidden" animate="show" custom={3} className={`${panel} mb-8`}>
            <div className={panelHead}>
            ...
            {subscription.billingHistory?.length > 0 ? (
              <ul className="divide-y divide-border-light dark:divide-border-dark">
                {subscription.billingHistory.map((payment) => (
                  <li key={payment.id} className="flex items-center justify-between px-5 py-3.5 gap-4">
                    <div className="flex items-center gap-3">
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="show" custom={4}>
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  className="shrink-0 px-4 py-2.5 text-[10px] font-mono tracking-eyebrow uppercase border border-red-500/30 text-red-500 dark:text-red-400 hover:bg-red-500/5 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500"
                >
                  Cancel
                </button>
              </div>
